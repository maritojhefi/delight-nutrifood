        ...
        <div class="p-2" style="max-height: 450px; overflow-y: auto;">
            @foreach ($ventas as $item)
                <div class="alert mb-1 {{ $item->productos->count() > 0 ? 'border-success alert-success' : 'alert-danger border-danger' }}@isset($cuenta) {{ $item->id == $cuenta->id ? 'solid alert-alt bordeado-pulse' : '' }} @endisset p-1 px-2 pb-2"
                    style="line-height: 0px" data-registro-venta-id="{{ $item->id }}">
                        ...
                        <div class="d-flex justify-content-between">
                            @switch ($item->tipo_entrega)
                                ...
                                @case ('recoger')
                                    <small
                                        class="@isset($cuenta) {{ $item->id == $cuenta->id ? 'text-white' : '' }} @endisset"><i
                                            class="fa fa-bolt"></i> Venta Rapida</small>
                                @break

                                @default
                                    <small class="@isset($cuenta) {{ $item->id == $cuenta->id ? 'text-white' : '' }} @endisset"><i class="fa fa-bolt"></i>
                                        Venta Rapida</small>
                                @break
                            @endswitch
                            ...
                            @if($item->reservado_at)
                                @php
                                    ...
                                    if ($diffInSeconds > 10800) { // 3 horas
                                        $colorClass = 'bg-dark';
                                    } elseif ($diffInSeconds > 1800) { // 30 minutos
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-3 m-0 p-0 letra12" style="max-height: 450px; overflow-y: auto;">

                        @foreach ($ventas->where('tipo_entrega', '!=', 'mesa') as $item)
                            @switch($item->tipo_entrega)
                                @case('recoger')
                                    <div class="alert alert-primary solid" style="cursor: pointer;" wire:click="seleccionar('{{ $item->id }}')" data-bs-dismiss="modal">
                                        <span><i class="fa fa-bolt " style="font-size: 18px !important;"></i> {{ $item->cliente ? Str::limit($item->cliente->name, 25) : $item->usuario_manual }} </span>
                                        <br>
                                        <small class="letra10">{{ GlobalHelper::timeago($item->created_at) }}</small>
                                        <strong class="badge badge-sm light badge-primary ms-1 float-end">{{ $item->totalFinal() }}
                                            Bs</strong>
                                ...
                                @break
                                @case('delivery')   
                                    <div class="alert alert-secondary solid" style="cursor: pointer;" wire:click="seleccionar('{{ $item->id }}')" data-bs-dismiss="modal">
                                        <span><i class="fa fa-truck " style="font-size: 18px !important;"></i> {{ $item->cliente ? Str::limit($item->cliente->name, 25) : $item->usuario_manual }} </span>
                                        <br>
                                        <small class="letra10">{{ GlobalHelper::timeago($item->created_at) }}</small>
                                        <strong class="badge badge-sm light badge-primary ms-1 float-end">{{ $item->totalFinal() }}
                                            Bs</strong>
                    ...
                    <div class="col-md-8 m-0 p-0 ps-2">
                        <div class="row">
                            @foreach ($mesas as $mesa)
                                ...
                                <div class="col-md-2 col-sm-4 col-3 mb-1"
                                    ...
                                    <div class="card mesa-card text-center p-1 {{ $mesaOcupada ? 'mesa-ocupada' : 'mesa-disponible' }}"
                                        ...
                                        @if ($mesaOcupada)
                                            onmouseover="this.style.borderColor='#dc3545'; this.style.transform='scale(1.05)'"
            ...
            <div class="modal-footer d-flex justify-content-between">
                ...
                <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal"
                    onclick="confirmarVentaDelivery()">Venta Delivery <i
