    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header row">
                ...
                <div class="col"> <input type="text" class="form-control form-control-sm" placeholder="Buscar"
                        wire:model.debounce.750ms="buscar"></div>
                                ...
                                <tr>
                                    ...
                                    @if ($item->descuento)
                                        <td><del
                                                ...
                                                Bs</span></td>
                                    @else
                                        <td><span class="w-space-no badge light badge-success">{{ $item->precio }}
                                                Bs</span></td>
                                    @endif
                                    <td><a href="#" wire:click="cambiarestado('{{ $item->id }}')">
                                            <div class="d-flex align-items-center"><i
                                                    class="fa fa-circle text-{{ $item->estado == 'activo' ? 'success' : 'danger' }} me-1"></i>
                                                {{ $item->estado }}</div>
                                        </a></td>
                                    <td><a href="#" wire:click="cambiarcontable('{{ $item->id }}')"><span
                                    ...
                                    <td><span class="w-space-no">
                                            ...
                                            <button class="btn btn-success light btn-xxs" data-bs-toggle="modal"
                                                data-bs-target="#modalgrande{{ $item->id }}">Ver stock</button>
                                    ...
                                    @if ($item->codigoBarra)
                                        <td><img src="data:image/png;base64,{{ DNS1D::getBarcodePNG($item->codigoBarra, 'C39+', 1, 33) }}"
                                                alt=""></td>
                                    @else
                                                    ...
                                                    @foreach ($item->sucursale as $stock)
                                                        <div class="card overflow-hidden bg-image-2 bg-secondary">
                                                            <div class="card-header  border-0">
                                                                <div>
                                                                    <small class="mb-2 font-w100 text-white">Lugar:
                                                                        {{ $stock->nombre }} - Vencimiento:
                                                                        {{ date('d-m-Y', strtotime($stock->pivot->fecha_venc)) }}</small>

                                                                    <h3 class="mb-0 fs-24 font-w600 text-white">
                                                                        Cantidad: {{ $stock->pivot->cantidad }}</h3>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                ...
                                                <div class="row">
                                                    <div class="col-4">
                                                        <input type="number" class="form-control"
                                                            wire:model="cantidadStock" placeholder="">
                                                    </div>
                                                    <div class="col-4">
                                                        <input type="date" class="form-control"
                                                            wire:model="fechaStock">
                                                    </div>
                                                    <div class="col-4">
                                                        <button wire:click="guardarStock({{ $item->id }})"
                                                            class="btn btn-sm btn-success">Agregar stock</button>
                                                    </div>
