...
@section('content')
<x-cabecera-pagina titulo="{{$producto->nombre}}" cabecera="bordeado"/>

    <div class="card card-style">
        <div class="card bg-13" data-card-height="250" style="height: 250px;">
        <div class="card-bottom pb-4 ps-3">
        <h1 class="font-26">@foreach ($nombrearray as $item){{$item}}<br>@endforeach</h1>
        </div>
        <div class="card-bottom pb-4 pe-3">
        <h1 class="font-30 text-end mb-3">{{$producto->descuento?$producto->descuento:$producto->precio}} Bs<sup class="font-400 font-17 opacity-50">({{$producto->medicion}})</sup></h1>
         <span class="badge bg-highlight color-white px-2 py-1 mt-n1 text-uppercase d-block float-end">{{$producto->descuento?'En descuento':'Sin promocion'}}</span>
        </div>
