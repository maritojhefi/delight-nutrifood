...
<div class="card card-style mb-3" id="accordion-3">
    <style>
        .btn-svg-bg {
            position: relative;
            overflow: hidden;
                            ...
                            <div class="d-inline-block">
                                <a href="#"
                                    ...
                                    <div class="position-relative text-center">
                                        <div class="fw-black"
                                            style="font-size: 1.8rem !important; color: #cc3f0a; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">
                                            {{ auth()->user()->convenios->first()->valor_descuento }}{{ auth()->user()->convenios->first()->tipo_descuento == 'porcentaje' ? '%' : 'Bs' }}
                                        </div>
                    ...
                    <div class="d-flex align-items-center mb-0">
                        <div class="row mb-0 ms-2">
                            <div class="col-12 col-md-12 text-center">
                                @if (auth()->user()->convenios->first()->tipo_descuento != 'porcentaje')
                                    <h5 class="font-700">Descuento de
                                        {{ auth()->user()->convenios->first()->valor_descuento }} Bs</h5>
                                @else
                            ...
                            <div class="row mb-0">
                                @foreach (json_decode(auth()->user()->convenios->first()->productos_afectados) as $productoId)
                                    @php
                                        $producto = GlobalHelper::obtenerModeloProducto($productoId);
                                        $convenioService = app(
                                            \App\Services\Ventas\Contracts\ConvenioServiceInterface::class,
                                        );
                                        $precioOriginal = $producto->precioReal();
                                        $precioFinal = $precioOriginal;
                                        if ($convenioService && auth()->user()) {
                                            $precioFinal = $convenioService->obtenerPrecioConDescuentoConvenio(
                                                $producto,
                                                auth()->user(),
                                            );
                                        }
                                    @endphp
                                    <a href="{{ route('detalleproducto', $productoId) }}">
                                        <p class="col-12 font-600 mb-0 border-bottom border-gray-200">
                                            ...
                                            <span class="ms-3">
                                                @if ($precioFinal < $precioOriginal)
                                                    <span
                                                        class="text-decoration-line-through color-red-light opacity-60 font-12 me-2">
                                                        {{ number_format($precioOriginal, 2) }} Bs
                                                    </span>
                                                    <span class="color-highlight font-600 font-14">
                                            ...
                                            <i class="ms-2 fa fa-hand-pointer fa-beat"></i>
                                        </p>
                                    </a>
                                @endforeach
                            </div>
