...
class AdminTicketsHelper
{
    ...
    public static function enviarMensajeConMenu($usuario, $paso, $menuDiaActual,$fechaPlan,$array)
    {
        ...
        switch ($paso) {
            ...
            case '4':
                ...
                if ($detalle->EMPAQUE != "Ninguno") {
                    $devolucion = WhatsappAPIHelper::enviarTemplate('delight_pedido_listo', [$diaPlan, $detalle->SOPA==''?'Sin sopa':$detalle->SOPA, $detalle->PLATO . '(' . $detalle->CARBOHIDRATO . ')', 'Carbohidrato: *' . $detalle->CARBOHIDRATO . '*', 'Metodo: *' . $detalle->ENVIO . '* ' . ' \nEmpaque: *' . $detalle->EMPAQUE . '*', 'Ingresa a tu perfil en nuestra pagina para personalizar toda tu semana o contactate con nosotros!'], $usuario->telf, 'es');
                    //dd($devolucion);
                } else {
        ...
        if ($numero != '0') {
            ...
            switch ($paso) {
                case '1':
                    ...
                    AdminTicketsHelper::enviarMensajeConMenu($usuario, $paso, $menuDiaActual,$fechaPlan,json_encode($array));
                    break;
                case '2':
                    if ($numero == "1") {
                ...
                case '3':
                    ...
                    if ($array->ENVIO != Plane::ENVIO1) {
                        ...
                        AdminTicketsHelper::enviarMensajeConMenu($usuario, '4', $menuDiaActual,$fechaPlan,json_encode($array));
